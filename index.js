const  XBOT = 'NA 5.0'; // Nama Bot Whatsapp
const instagram = ''; // Nama Instagramlu cok
const nomer = ''; // Nomor whatsapplu cok
const aktif = 'Dependendo da cota'; // Kapan bot lu aktif
...
conn.sendMessage(group.gid, "olaaa", MessageType.extendedText) // say hello to everyone on the group
...
conn.sendMessage(id, 'Owner XBOT ' ,MessageType.text);
}
else if (text == 'affis'){
  ...
  var teks = text.replace(/#nulis /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '[ESPERE] Pesquisando...', MessageType.text)
  ...
  var teks = text.replace(/%nulis2 /, '')
    axios.get(`https://mhankbarbars.herokuapp.com/nulis?text=${teks}&apiKey=N2Ws9kp3KTDYtry5Jjyz`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, 'Bot  Nulis 📝', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
...
conn.sendMessage(id, 'nome trocado com sucesso' ,MessageType.text, { quoted: m } );
...
conn.sendMessage(id, 'descri��o alterada com sucesso' ,MessageType.text, { quoted: m } );

}
if (text.includes('#creator')){
conn.sendMessage(id, {displayname: "Jeff", vcard: vcard}, MessageType.contact)
conn.sendMessage(id, 'na', MessageType.text)
}
...
conn.sendMessage(id, 'SUCESSO, O GRUPO FOI ABERTO' ,MessageType.text, { quoted: m } );
}
else if (text == '#closegc'){
 let hasil = `${id.split("@s.whatsapp.net")[0]}`;
   conn.groupSettingChange (hasil, GroupSettingChange.messageSend, true);
conn.sendMessage(id, 'SUCESSOS, O GRUPO FOI FECHADO' ,MessageType.text, { quoted: m } );
}
...
if (text.includes("#ytmp4"))
   {
      ...
      {
         conn.sendMessage(id, "Desculpe, o link que voc� enviou � inv�lido", MessageType.text)
      }
      ytdl.getInfo(videoid[1]).then(info =>
      {
         if (info.length_seconds > 1000)
         {
            conn.sendMessage(id, " O v�deo � longo", MessageType.text)
         }
         else
         {
